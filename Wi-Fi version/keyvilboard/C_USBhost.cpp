...
void C_USBhost::SaveTheKeys(){
  if(WasAnyKeyPressed()){
    ...
    for(int i = 0; i < sizeof(keys_pressed); i++)
    {
       ...
       if(key){
        if(debug){
          Serial.print(F("\nFirst block rawHID byte list: ")); Serial.println(rawHID[0]);
          Serial.print(F("rawHID string: ")); Serial.println(hidText);                                                                       // debug line
          Serial.print(F("rawHID key detected: hex - ")); Serial.print(key, HEX); Serial.print(F(", int - ")); Serial.println((int)key);       // debug line
        }
        
        byte asciiKey = HID_to_ASCII(key, WasShiftDown(), WasWindowsDown(), WasAltDown(), WasCtrlDown());
...
byte C_USBhost::HID_to_ASCII(byte key, bool shiftDown, bool windowsDown, bool altDown, bool ctrlDown){ //add feature for ctrl, windows and alt combo detection
...
bool C_USBhost::WasShiftDown() {
  return (IsBitHigh(rawHID[0], 1) || IsBitHigh(rawHID[0], 5)); // if 1nd or 5th bit (left/right shift) shift 2 OR 32 => 010001
}

bool C_USBhost::WasAltDown() {
  return (IsBitHigh(rawHID[0], 2) || IsBitHigh(rawHID[0], 6)); // if 2nd or 6th bit (left/right alt) alt 4 OR 64 => 0010001
}

bool C_USBhost::WasWindowsDown() { 
  return (IsBitHigh(rawHID[0], 3) || IsBitHigh(rawHID[0], 7)); // if 3nd or 7th bit (left/right windows) windows 8 OR 128 => 00010001
}

bool C_USBhost::WasCtrlDown() {
  return (IsBitHigh(rawHID[0], 0) || IsBitHigh(rawHID[0], 4)); // if 0nd or 4th bit (left/right windows) windows 1 OR 16 => 10001
}
